...
if (Meteor.isServer) {
  describe('ProfileCollection', function testSuite() {
    ...
    const defineObject = {
      firstName, lastName, username, bio, interests, picture, title, location, github, facebook, instagram,
    };
    ...
    it('#define (duplicate interests)', function test() {
      ...
      const defineObject3 = {
        firstName, lastName, username, bio, interests: duplicateInterests, picture, title, location,
        github, facebook, instagram,
      };
      expect(function foo() {
